/* eslint-disable @typescript-eslint/no-explicit-any -- frontend component with flexible callback types */
...
export default function initPagination({
  ...
  const mountEl =
    typeof mount === 'string' ? (document.querySelector(mount) as HTMLElement) : mount;
  ...
  function buildButton(label: string, page: number, disabled = false, active = false) {
  ...
  function renderControls() {
    ...
    if (end < totalPages) {
      ...
      container.appendChild(
        buildButton(String(totalPages), totalPages, false, currentPage === totalPages),
      );
    }
