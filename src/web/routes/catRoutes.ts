...
export function setupCatRoutes(
  ...
  router.get('/cat/:id', async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  router.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        res.json(toLeaderboardDto(rows));
      } catch (err) {
        next(err);
      }
    },
  ...
  router.get('/similar', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const cats = await catInfoService.getCatsByFeature(feature as string, value as string);
      res.json(cats);
    } catch (err) {
      next(err);
    }
  ...
  router.get('/random-images', async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  router.post('/like', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (result === false) {
        // Usually means already liked or cat not found, but for idempotency we can return 200 or 409
        // Let's return 200 with a message
        return res.json({ success: false, message: 'Already liked or failed' });
      }
